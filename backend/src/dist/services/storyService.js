...
exports.mintLicenseOnFlow = exports.registerIpWithFlow = exports.checkExistingLicenses = void 0;
...
const registerIpWithFlow = async (ipHash, metadata, isEncrypted, modredIpContractAddress) => {
    const maxRetries = 3;
    let lastError = null;
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            lastError = error;
            console.error(`❌ Error registering IP with Flow (attempt ${attempt + 1}/${maxRetries}):`, error?.message || error);
...
exports.registerIpWithFlow = registerIpWithFlow;
const mintLicenseOnFlow = async (tokenId, royaltyPercentage, duration, commercialUse, terms, modredIpContractAddress) => {
    const maxRetries = 3;
    let lastError = null;
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            lastError = error;
            console.error(`❌ Error minting license on Flow (attempt ${attempt + 1}/${maxRetries}):`, error?.message || error);
            // Check if it's a nonce-related error (check nested cause chain)
            const checkNonceError = (err, depth = 0) => {
                if (!err || depth > 5)
                    return false; // Prevent infinite recursion
...
exports.mintLicenseOnFlow = mintLicenseOnFlow;
